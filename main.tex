\documentclass[linenumbers, trackchanges, tighten]{aastex61}%twocolumn,
...
In the top panel of \Fig{CSFHQ} we compare the time evolution of \QHat{} for instantaneous bursts and constant SFR populations. The constant SFR models reach an equilibrium around 4 Myr, after which the ionizing photon rate is essentially constant. The bottom panel of \Fig{CSFHQ} shows the time evolution of the EUV slope. The slope of the constant-SF model eventually reaches equilibrium around 6 Myr, with a slope that is roughly equivalent to that of a 2 Myr instantaneous burst. \added{We note that these identified equilibrium ages correspond to a specific IMF and set of evolutionary tracks and may be different for stellar populations generated with different properties}.
...
\begin{figure*}[!htbp]
 \begin{centering}
    ...
    \caption{The nebular continuum emission \added{is a combination of bound-free, free-free and two-photon emission processes}. The dashed lines indicate the wavelength of various hydrogen series limits ($n=\infty \rightarrow \{1,2,3,\ldots\}$). \textbf{Top:} Continuum emission spectrum for a 1 and 3 Myr model at solar metallicity and varying \U{}. \replaced{The free-bound continuum is a recombination process and the intensity of emission scales with ionization parameter, we have removed the \U{} dependence by normalizing the continuum by $\QHat/\QH$.}{The free-bound continuum is a recombination process, thus the intensity of continuum emission scales with model ionization parameter. To remove the \U{} dependence we normalize the continuum by $\QHat/\QH$.} \textbf{Bottom:} Normalized continuum emission spectrum as a function of model metallicity for a 3 Myr SSP at $\logU=-1.0$ for metallicities between -1.0 and solar. The slope and sharpness of the recombination edges varies in response to the thermalization of electrons to local gas temperature. \replaced{Each metallicity is color-coded by the volume averaged model temperature.}{To emphasize the relationship between gas temperature and gas metallicity, each metallicity is color-coded by \Te{}, the volume-averaged electron temperature of the gas.}}
    \label{fig:nebCont}
 \end{centering}
\end{figure*}
%-------------------------------------------------------
